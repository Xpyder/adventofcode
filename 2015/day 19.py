...
filename = Path(__file__).stem+".txt"
...
def solve(data):
    if part_1:
        ...
        return how_many_single_replacements(molecule,replacements)
    else:
...
def how_many_single_replacements(starting_molecule: str,replacements: list[tuple]):
    possible_molecules = set()
    for frm,to in replacements:
        for match in re.finditer(frm,starting_molecule):
            molecule = starting_molecule
            match_start,match_end = match.span()
            molecule = molecule[:match_start] + to + molecule[match_end:]
            possible_molecules.add(molecule)
